...
class CLIPImageSimilarity_local:
...
class Img2Mask_local:
    def __init__(self, dinox_sam2_clip_config):
        ...
        self.grounding_model = load_model(
            model_config_path=dinox_sam2_clip_config.grounding_dino_config,
            model_checkpoint_path=dinox_sam2_clip_config.grounding_dino_checkpoint,
            device=self.device
        )
        self.dinox_sam2_clip_config = dinox_sam2_clip_config
    ...
    def detect_objects(self, image_path: str):
        ...
        boxes, confidences, labels = predict(
            ...
            caption="<prompt_free>",
            box_threshold=0.05,
            text_threshold=self.dinox_sam2_clip_config.text_threshold,
        )
...
def find_most_similar_local(clip_masks,object_name,img2mask,clip_similarity):
    max_similarity = 0
    ...
    for index, clip_mask in enumerate(clip_masks):
        ...
        # clip_mask_area = calculate_area(clip_mask)
        # object_mask_area = calculate_area(img2mask.image_dict[object_name])

        area_ratio = calculate_area_ratio(clip_mask, img2mask.image_dict[object_name])

        similarity = 0.5 * c_similarity + 0.5 * area_ratio
        # 得到clip_mask有效
